 ...
 *
 *  Created by Sandru Andrei on 10/22/2018
 */
...
(function(){
    ...
    PIXI.TextInput.prototype.keyInput = function(e)
    {
        ...
        if (typeof evt.which == "number" && evt.which > 0 && !e.ctrlKey && !e.metaKey && !e.altKey) {
            if (e.data.stage instanceof PIXI.Stage && e.data.stage.interactionManager.mouse.target === e.data) {
                e.preventDefault();
                e.stopPropagation();
                if (e.which === 8) {
                    e.data.backspace();
