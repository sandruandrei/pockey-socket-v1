// Type definitions for GreenSock GS_Animation Platform 1.15.1
...
declare class GS_Animation {
    ...
    delay(): number;
    delay(value: number): GS_Animation;

    duration(): number;
    duration(value: number): GS_Animation;

    eventCallback(type: string): Function;
    eventCallback(type: string, callback: Function, params?: any[], scope?: any): GS_Animation;

    invalidate(): GS_Animation;

    isActive(): boolean;

    kill(vars?: Object, target?: Object): GS_Animation;

    pause(atTime?: any, suppressEvents?: boolean): GS_Animation;

    paused(): boolean;
    paused(value: boolean): GS_Animation;

    play(from?: any, suppressEvents?: boolean): GS_Animation;

    progress(): number;
    progress(value: number, supressEvents?: boolean): GS_Animation;

    restart(includeDelay?: boolean, suppressEvents?: boolean): GS_Animation;

    resume(from?: any, suppressEvents?: boolean): GS_Animation;

    reverse(from?: any, suppressEvents?: boolean): GS_Animation;

    reversed(): boolean;
    reversed(value: boolean): GS_Animation;

    seek(time: any, suppressEvents?: boolean): GS_Animation;

    startTime(): number;
    startTime(value: number): GS_Animation;

    time(): number;
    time(value: number, suppressEvents?: boolean): GS_Animation;

    timeScale(): number;
    timeScale(value: number): GS_Animation;

    totalDuration(): number;
    totalDuration(value: number): GS_Animation;

    totalProgress(): number;
    totalProgress(value: number): GS_Animation;

    totalTime(): number;
    totalTime(time: number, suppressEvents?: boolean): GS_Animation;
}

declare class SimpleTimeline extends GS_Animation {
...
//com.greensock
declare class TweenLite extends GS_Animation {
